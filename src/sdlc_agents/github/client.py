...
class GithubClient:
    ...
    def get_pr_diff(self, pr_number: int, max_chars: int = 300_000) -> str:
        pr: PullRequest = self.repo.get_pull(number=pr_number)
        # PyGithub doesn't provide raw diff directly with a simple call in all versions;
        # easiest is to use pr.diff_url and requests with token, but that adds deps.
        # We'll approximate using patch fields from files:
        parts: list[str] = []
        for f in pr.get_files():
            if f.patch:
